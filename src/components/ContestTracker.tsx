...
import { ChevronUp, Globe, Plus, RefreshCw, Trophy } from 'lucide-react';
      ...
      <CardHeader className="px-0 pb-4 pt-0">
        <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            <Button
              variant={showAllContests ? 'default' : 'outline'}
              size="sm"
              className="bg-background/50 backdrop-blur-sm"
              onClick={() => setShowAllContests(v => !v)}
            >
              {showAllContests ? (
              ...
              <span className="hidden sm:inline">{showAllContests ? 'Hide Contests' : 'All Contests'}</span>
              <span className="sm:hidden">{showAllContests ? 'Hide' : 'All'}</span>
            </Button>
